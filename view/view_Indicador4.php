...
$(document).ready(function(){
 ...
 $('#txtfe').attr('readonly', true);
  ...
  $("#Btnnuevo").click(function (){
    ...
    $('#txtfe').datepicker('enable'); $('#cbtcentro').attr('disabled', false);  $("#idmensaje1").removeClass("alerta1").html(''); 
    $("#popupWindow").jqxWindow('show');       
  }); 
  $("#popupWindow").jqxWindow({ 
    width: 650, height: '70%', resizable: false,  theme: theme,  isModal: true,  autoOpen: false,  modalOpacity: 0.5  
  }); 
...
<?php $timestamp = time();?>
  ...
  $('#btngraba').click(function() {
    ...
    $.post("<?php echo $urlControles;?>",{ 
      tipf : '0001' , txtfe : $("#txtfe").val(), cbtcentro : $("#cbtcentro").val(),
...
function EventoDetalle (row) {
  ...
  var dataRecord = $("#jqxgrid").jqxGrid('getrowdata', indice);
    ...
    $("#txtfe").val(dataRecord.reg_fec); $('#txtfe').attr('readonly', true);  $('#txtfe').datepicker().datepicker('disable');  $('#cbtcentro').attr('disabled', true);  $("#txtnin1").val(dataRecord.nro1); $("#txtnin2").val(dataRecord.nro2);    
    $("#id_upt").val(dataRecord.codId);  $("#fecreg").val(dataRecord.fec_reg);  $("#idmensaje1").html('');
    $("#popupWindow").jqxWindow('show'); 
}
...
function Eventoelimina (row) {
  ...
  var dataRecord = $("#jqxgrid").jqxGrid('getrowdata', indice);
    ...
    $.post("<?php echo $urlControles;?>",{ 
    codid : dataRecord.codId, fec_reg:dataRecord.fec_reg , tipf : '0002', acttoken:$('#acttoken').val(),  rand:Math.random() } ,                
    function(data){
      if(data.trim() == "ok"){
        //document.location = '?opm=MsgText';
        $("#jqxgrid").jqxGrid('updatebounddata');
...
<?php
$titulo = html_entity_decode('Indicador 4 :   PORCENTAJE DE NIÑOS Y NIÑAS MENORES DE 1 AÑO DE EDAD CON CONTROL CRED Y SUPLEMENTO DE HIERRO O MULTIMICRONUTRIENTES'  )
?>
        ...
        <div style="overflow: hidden;">
            <center><b> <div class="poptitulo"></div> </b></center>
             <div class="miscrol"> 
              ...
              <tr>
                <td  colspan='2' > <?php echo html_entity_decode ('Número de  niños de 6 a 11 meses de edad que recibieron hierro');?>  : 
                	<input type="text" id="txtnin1" name="txtnin1" class="numero" maxlength='4' /></td>
                <td  colspan='2' > <?php echo html_entity_decode ('Numero de Niños de 6 a 11 meses ');?>  : 
